...
class TestCourierCreate:
    ...
    def test_fail_create_two_similar_courier(self):
        payload = helpers.create_new_courier()
        requests.post(url.URL_BASE + url.URL_COURIER, data=payload)
        response = requests.post(url.URL_BASE + url.URL_COURIER, data=payload)
    ...
    def test_fail_create_courier_without_login(self):
        response = requests.post(url.URL_BASE + url.URL_COURIER, data=data.TestBodyData.body_data_without_login)
    ...
    def test_fail_create_courier_without_password(self):
        response = requests.post(url.URL_BASE + url.URL_COURIER, data=data.TestBodyData.body_data_without_password)
        assert (response.status_code == 400 and
                response.json()['message'] == data.TestBodyText.text_courier_400)
...
class TestCourierLogin:
    ...
    def test_fail_authorisation_with_non_existent_user(self):
        response = requests.post(url.URL_BASE + url.URL_LOGIN, data=data.TestBodyData.body_data_authorisation)
